...
public:	
	...
	CVX_MaterialVoxel(double youngsModulus=1e6, double density=1e3, double nominalSize=0.001); //!< Default Constructor. @param[in] youngsModulus The Young's Modulus (stiffness) of this material in Pascals. @param[in] density The density of this material in Kg/m^3. @param[in] nominalSize The nominal voxel size in meters.
	...
	//mass and inertia
	double mass(){return _mass;} //!<Returns the mass of a voxel of this material in Kg. Mass cannot be specified directly. Mass is indirectly calculated according to setDensity() and setBaseSize().
	double momentInertia(){return _momentInertia;} //!<Returns the first moment of inertia of a voxel of this material in kg*m^2. This quantity is indirectly calculated according to setDensity() and setBaseSize().

	//damping convenience functions
	double internalDampingTranslateC() const {return zetaInternal*_2xSqMxExS;} //!< Returns the internal material damping coefficient (translation).
	double internalDampingRotateC() const {return zetaInternal*_2xSqIxExSxSxS;} //!< Returns the internal material damping coefficient (rotation).
	double globalDampingTranslateC() const {return zetaGlobal*_2xSqMxExS;} //!< Returns the global material damping coefficient (translation)
	double globalDampingRotateC() const {return zetaGlobal*_2xSqIxExSxSxS;} //!< Returns the global material damping coefficient (rotation)
	double collisionDampingTranslateC() const {return zetaCollision*_2xSqMxExS;} //!< Returns the global material damping coefficient (translation)
	double collisionDampingRotateC() const {return zetaCollision*_2xSqIxExSxSxS;} //!< Returns the global material damping coefficient (rotation)

	//stiffness
	double penetrationStiffness() const {return (double)(2*E*nomSize);} //!< returns the stiffness with which this voxel will resist penetration. This is calculated according to E*A/L with L = voxelSize/2.
...
protected:
	...
	void setGravityMultiplier(double gravityMultiplier){gravMult = gravityMultiplier;} //!< Sets the multiple of gravity for this material. In normal circumstances this should only be called by the parent voxelyze object. @param[in] gravityMultiplier Gravity multiplier (1 = 1G gravity).
	double gravityMuliplier() {return gravMult;} //!< Returns the current gravity multiplier.
	double gravityForce() {return -_mass*9.80665*gravMult;} //!< Returns the current gravitational force on this voxel according to F=ma.
	...
	double gravMult; //!< Multiplier for how strongly gravity should affect this block in g (1.0 = -9.81m/s^2)
	double _mass; //!< Cached mass of this voxel (kg)
	double _massInverse; //!< Cached 1/Mass (1/kg)
	double _sqrtMass; //!< Cached sqrt(mass). (sqrt(Kg))
	double _firstMoment; //!< Cached 1st moment "inertia" (needed for certain calculations) (kg*m)
	double _momentInertia; //!< Cached mass moment of inertia (i.e. rotational "mass") (kg*m^2)
	double _momentInertiaInverse; //!< Cached 1/Inertia (1/(kg*m^2))
	double _2xSqMxExS; //!< Cached value needed for quick damping calculations (Kg*m/s)
	double _2xSqIxExSxSxS; //!< Cached value needed for quick rotational damping calculations (Kg*m^2/s)
