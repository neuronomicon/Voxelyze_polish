...
public:
	...
	std::vector<Vec3D<double>> cornerVecs;

	std::vector<Vec3D<double>> cornerVelocity;
	bool is_Any_Corner_Exposed;
	...
	void timeStep(double dt); //!< Advances this voxel's state according to all forces and moments acting on it. Large timesteps will cause instability. Use CVoxelyze::recommendedTimeStep() to get the recommended largest stable timestep. @param[in] dt Timestep (in second) to advance.
	...
	// Added by Shim - 2025-01-12 01:02AM
	Vec3D<double> originPos;
	void Set_OriginPos( void )
	{
		double s = mat->nominalSize();
		originPos = Vec3D<double>( (double)ix*s, (double)iy*s, (double)iz*s );
	}
	...
	Quat3D<double> originOrient;
	Quat3D<double> original_Orient()
	{	
		return originOrient;
	}
	...
	Vec3D<double> cornerPosition(voxelCorner corner) const; //!< Returns the deformed location of the voxel corner in the specified corner in the global coordinate system (GCS). Essentially cornerOffset() with the voxel's current global position/rotation applied.
	Vec3D<double> cornerOffset(voxelCorner corner) const; //!< Returns the deformed location of the voxel corner in the specified corner in the local voxel coordinate system (LCS). Used to draw the deformed voxel in the correct position relative to the position().
	...
	Vec3D<double> baseSize() const {return mat->size()*(1.0+temp*mat->alphaCTE);} //!<Returns the nominal size of this voxel (LCS) accounting for any specified temperature and external actuation. Specifically, returns the zero-stress size of the voxel if all forces/moments were removed.
	double baseSize(CVX_Link::linkAxis axis) const {return mat->size()[axis]*(1.0+temp*mat->alphaCTE);} //!<Returns the nominal size of this voxel in the specified axis accounting for any specified temperature and external actuation. Specifically, returns the zero-stress dimension of the voxel if all forces/moments were removed.
	double baseSizeAverage() const {Vec3D<double> bSize=baseSize(); return (bSize.x+bSize.y+bSize.z)/3.0;} //!<Returns the average nominal size of the voxel in a zero-stress (no force) state. (X+Y+Z/3)
	...
	double orientationAngle() const {return (double)orient.Angle();} //!< Use with orientationAxis() to get the orientation of this voxel in angle/axis form. Returns the angle in radians.
	...
	double displacementMagnitude() const {return (double)displacement().Length();} //!< Returns the distance (magnitude of displacement) this voxel has moved from its initial nominal position. (GCS)
	double angularDisplacementMagnitude() const {return (double)orient.Angle();} //!< Returns the angle (magnitude of angular displacement) this voxel has rotated from its initial nominal orientation. (GCS)
	Vec3D<double> velocity() const {return linMom*mat->_massInverse;} //!< Returns the 3D velocity of this voxel in m/s (GCS)
	double velocityMagnitude() const {return (double)(linMom.Length()*mat->_massInverse);} //!< Returns the velocity of this voxel in m/s.
	...
	double angularVelocityMagnitude() const {return (double)(angMom.Length()*mat->_momentInertiaInverse);} //!< Returns the angular velocity of this voxel in rad/s.
	double kineticEnergy() const {return (double)(0.5*(mat->_massInverse*linMom.Length2() + mat->_momentInertiaInverse*angMom.Length2()));} //!< Returms the kinetic energy of this voxel in Joules.
	double volumetricStrain() const {return (double)(strain(false).x+strain(false).y+strain(false).z);} //!< Returns the volumetric strain of the voxel according to the definition at http://www.colorado.edu/engineering/CAS/courses.d/Structures.d/IAST.Lect05.d/IAST.Lect05.pdf
	double pressure() const 
	{
		return -mat->youngsModulus()*volumetricStrain() / ( 3.0*(1.0-2.0*mat->poissonsRatio()) );
	...
	double temperature() {return temp;} //!< Returns the current temperature of this voxel in degrees Celsius.
	...
	Vec3D<double> externalForce(); //!< Returns the current external force applied to this voxel in newtons. If the voxel is not fixed this will return any applied external forces. If fixed it will return the current reaction force necessary to enforce the zero-motion constraint.
	Vec3D<double> externalMoment(); //!< Returns the current external moment applied to this voxel in N-m. If the voxel is not fixed this will return any applied external moments. If fixed it will return the current reaction moment necessary to enforce the zero-motion constraint.
	...
	double floorPenetration() const 
	{
	//	double ghgh = (double)(baseSizeAverage()/2 - mat->nominalSize()/2 - pos.z);
	//	double hhff = 0.5*(baseSizeAverage() - mat->nominalSize() ) - pos.z;

		return 0.5*(baseSizeAverage() - mat->nominalSize() ) - pos.z;
	...
	double dampingMultiplier() {return 2*mat->_sqrtMass*mat->zetaInternal/previousDt;} //!< Returns the damping multiplier for this voxel. This would normally be called only internally for the internal damping calculations.
...
private:
	...
	void floorForce(double dt, Vec3D<double>* pTotalForce); //modifies pTotalForce to include the object's interaction with a floor. This should be calculated as the last step of sumForce so that pTotalForce is complete.
	...
	void eulerStep(double dt); //execute an euler time step at the specified dt
	double previousDt; //remember the duration of the last timestep of this voxel

	void updateSurface();
	void enableCollisions(bool enabled, double watchRadius = 0.0); //watchRadius in voxel units
