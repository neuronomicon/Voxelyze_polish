...
public:
	...
	void Rotate_VoxelBot( double theta , double hori , double vert );
	int Find_Center_Voxel_Index( void );

	// Added by OSM //////////////////////////////////////////////////
	void SliceVoxelLinkSample(CVX_Voxel* Voxel1, CVX_Voxel* Voxel2);
	...
	bool doLinearSolve(/*SOLVER thisSolver, double stepPercentage = 1.0*/); //!< Linearizes the voxelyze object and does a one-time linear solution to set the position and orientation of all voxels. The current state of the voxel object will be discarded. Currently only the pardiso solver is supported. To make use of this feature voxelyze must be built with PARDISO_5 defined in the preprocessor. A valid pardiso 5 license file and library file (i.e libpardiso500-WIN-X86-64.dll for windows) should be obtained from www.pardiso-project.org and placed in the directory your executable will be run from.

	bool doTimeStep(double dt = -1.0); //!< Executes a single timestep on this voxelyze object and updates all state information (voxel positions and orientations) accordingly. In most situations this function will be called repeatedly until the desired result is obtained. @param[in] dt The timestep to take in seconds. If this value is too large the system will display divergent instability. Use recommendedTimeStep() to get a conservative estimate of the largest stable timestep. Also the default value of -1.0 will blindly use this recommended timestep.
	double recommendedTimeStep() const; //!< Returns an estimate of the largest stable time step based on the current state of the simulation. If poisson's ratios are all zero and material properties do not otherwise change this can be called once and the same timestep value used for all subsequent doTimeStep() calls. Otherwise the timestep should be recalculated whenever the simulation has changed.
	...
	//temperature
	void setAmbientTemperature(double relativeTemperature, bool allVoxels = false); //!< Sets the relative ambient temperature of the environment. A relative temperature of zero indicates that materials will experience zero thermal expansion. @param[in] relativeTemperature Relative temperature in degrees celsius. @param[in] allVoxels Flag to denote whether all voxels should take this temperature immediately. If true any diffusion time is skipped and thermal energy is added (or removed) from each voxel to reach this temperature. If false voxels will not change temperature.
	double ambientTemperature(void) const {return ambientTemp;} //!< Returns the current relative ambient temperature

	void setGravity(double g=1.0); //!< Sets the gravitational acceleration to apply to all voxels. Gravity acts in the -Z direction. Set to 0 to disable. @param[in] g Gravitational acceleration in g's. 1 g = -9.80665 m/s^2.
	double gravity(void) const {return grav;} //!< Returns the current gravitational acceleration in g's. 1 g = -9.80665 m/s^2.
	...
	//info
	double stateInfo(stateInfoType info, valueType type); //!< Returns a specific piece of information about the current state of the simulation. This method is not gaurenteed threadafe. @param[in] info The class of information desired. @param[in] type The type of the value to be returned.
...
private:
	...
	double boundingRadius; //(in voxel units) radius to collide a voxel at
	double watchDistance; //(in voxel units) Distance between voxels (not including 2*boundingRadius for each voxel) to watch for collisions from.
...
public:
	bool doTimeStep_Nested(double dt = -1.0);
	void updateCollisions_Nested();
};
