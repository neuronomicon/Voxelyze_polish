...
bool CVoxelyze::doTimeStep_Nested(double dt)
{
   ...
   /*     int do_coll = 0;
		...
		// 팀 전체 호출
        if (do_coll)	updateCollisions_Nested();
...
*/
		...
		// [Phase 3] 뒤 암묵적 배리어에서 diverged_flag 가 확정된다
		const int diverged = diverged_flag;        // 전 스레드 동일값

		if (!diverged && collisions)
			updateCollisions_Nested();             // single/copyprivate 불필요
...
{
    const double watchRadiusVx  = 2.0 * boundingRadius + watchDistance;
    const double watchRadiusMm  = (double)(voxSize * watchRadiusVx);
    ...
    // 안전장치: parallel 밖에서 호출된 경우 (변경 없음)
    if (!omp_in_parallel())
    {
        if (nearbyStale) {
            for (int i = 0; i < voxCount; i++)
    ...
    // 1)+2)+3) 힙을 건드리는 전부를 master 에 고정.
    //   single → master 로 바꾼 이유:
    //     single 은 "먼저 도착한 스레드"가 실행하므로 스텝마다 실행자가 바뀐다.
    //     그러면 new 는 A 스레드, 다음 스텝의 delete 는 B 스레드에서 일어나
    //     Windows LFH 가 할당 슬롯으로 블록을 회수하지 못하고 서브세그먼트를
    //     계속 새로 확보한다 (= inner thread >= 2 에서만 나타나는 메모리 증가).
    //   master 는 이 parallel 을 만든 outer 스레드 = 로봇별로 세션 내내 고정.
    //
    //   스테일 체크(516회 거리 비교, 1us 미만)도 여기 흡수했다.
    //   병렬화해서 얻는 이득보다 omp for 배리어 + critical 비용이 크다.
    #pragma omp master
    {
        if (nearbyStale)
        {
    ...
    #pragma omp barrier     // master 는 암묵적 배리어가 없다  반드시 필요

    // 배리어 뒤이므로 shared 멤버를 그냥 읽어도 전 스레드가 같은 값을 본다.
    // (copyprivate 불필요 — master 에서는 쓸 수도 없다)
    const int colCount = (int)collisionsList.size();

    // 4) contact force — 병렬 유지
    //    updateContactForce() 는 this->force 만 쓰고 복셀은 읽기만 하므로
    //    충돌 객체 1개 = 스레드 1개 소유가 되어 레이스가 없다.
    #pragma omp for schedule(static)
    for (int i = 0; i < colCount; i++)
        collisionsList[i]->updateContactForce();

    // omp for 의 암묵적 배리어: Phase 4 의 contactForce() 읽기 전에 force 확정
}
